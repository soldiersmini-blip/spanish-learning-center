    ...
    >
      <header className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
        <div className="min-w-0">
          <p className="text-[11px] font-black uppercase tracking-widest text-coral-600 dark:text-coral-100">Neural Link</p>
          <h2 className="truncate text-base font-black text-slate-950 dark:text-white">{node?.title || '未找到节点'}</h2>
        </div>
        <button
          type="button"
          onClick={onClose}
          className="rounded-xl border border-slate-200 p-2 text-slate-500 hover:bg-slate-100 hover:text-coral-700 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
          title="收起 Neural Link"
        >
          <X className="h-4 w-4" />
        </button>
      </header>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        {node ? <KnowledgeRail initialNode={node} /> : <p className="text-sm text-slate-500">这个节点暂时不存在。</p>}
      </div>
    </aside>
  );
}

function KnowledgeRail({ initialNode }: { initialNode: NeuralNode }) {
  const [activeId, setActiveId] = useState(initialNode.id);
  const [openSections, setOpenSections] = useState<string[]>(['usage', 'contrast', 'grammar', 'path']);
  const tree = getKnowledgeTree(activeId) || getKnowledgeTree(initialNode.id);
  ...
  if (!tree) return <p className="text-sm text-slate-500">这个节点暂时没有知识树。</p>;
